...
function initYears(adblockers) {
  ...
  for (var id in adblockers) {
    ...
    var start = new Date(adblocker.started || adblocker.releases[0][0]).getFullYear();
    if (!startYear || start < startYear) {
      startYear = start;
    }
    
    var end = new Date(adblocker.ended || adblocker.releases[adblocker.releases.length - 1][0]).getFullYear();
...
module.exports.getSVG = function(callback) {
  getData(function(adblockerData) {
    ...
    getTemplate(function(template) {
      var date = new Date();
      var years = initYears(adblockers);
      var width = years.length * 120;
